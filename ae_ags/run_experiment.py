...
PRESETS = {
    ...
    # Same hyperparameters as paper_default, but per-round stable regret is max(μ_ref−X, 0), so cumulative
    # regret in Appendix Fig.1(a)–(e) stays nonnegative (paper-style plots often sit in the nonnegative orthant).
    "paper_default_rectified": {
        "N": 5,
        "K": 5,
        "T": 100000,
        "delta": 0.1,
    ...
    # Appendix Fig.1 (E) at C-ETC scale θ≈ln-coeff·ln(T)/Δ² with θ=8.35: same noise/market as paper_default,
    # but AE-AGS uses README full-horizon funnel knobs so cumulative unstability is competitive with C-ETC.
    # Theorem-clean AE (cf=6, pick_one, random) stays on preset paper_default.
    "paper_appendix_e_fig1": {
        "N": 5,
        "K": 5,
        "T": 100000,
        "delta": 0.1,
    ...
    "paper_fig1_knee15k": {
        ...
        "record_every": 1000,
        "market_model": "paper_rank",
        # Under θ=2.5 scans (see results/paper_run/plan_appendix_e/fig1_knob_scan_knee15k_T25k_r5.txt),
        # pick_one + random dominates round_sweep + smallest_arm for mean AE unstability at medium T.
        "aeags_confidence_factor": 5.0,
        "c_etc_log_coeff": 2.5,
        "p_etc_explore_coef": 0.52,
        "aeags_arm_schedule": "fixed",
    ...
    # paper_fig1_knee15k with nonnegative cumulative stable regret (panels (a)–(e)); panel (f) unchanged.
    "paper_fig1_knee15k_rectified": {
        "N": 5,
        "K": 5,
        "T": 100000,
        "delta": 0.1,
